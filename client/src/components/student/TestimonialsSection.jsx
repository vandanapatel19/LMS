...
const TestimonialsSection = () => {
  return (
    <div className='pb-14 px-8 md:px-0'>
      ...
      <div className='grid grid-cols-(--grid-col) gap-8 mt-14'>
        {dummyTestimonial.map((testimonal, index) => (
          <div className='text-sm text-left border border-gray-500/30 pb-6 rounded-lg bg-white shadow-[0px_4px_15px_0px] shadow-black/5 overflow-hidden' key={index}>
            <div className='flex items-center gap-4 px-5 py-4 bg-gray-500/10'>
              <img className='h-12 w-12 rounded-full' src={testimonal.image} alt={testimonal.name} />
              <div>
                <h1 className='text-lg font-medium text-gray-800'>{testimonal.name}</h1>
                <p className='text-gary-800/80'>{testimonal.role}</p>
              </div>
            </div>
            <div className='p-5 pb-7'>
              <div className='flex gap-0.5'>
                {[...Array(5)].map((_, i) => (
                  <img className='h-5' key={i} src={i < Math.floor(testimonal.rating) ? assets.star : assets.star_blank} alt='' />
                ))}
              </div>
