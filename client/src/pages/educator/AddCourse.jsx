  ...
  return (
    <div className="h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pb-0">
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        ...
        <div className="flex flex-col gap-1">
          <p>Discount %</p>
          <input onChange={e => setDiscount(e.target.value)} value={discount} type="number" placeholder='0' min={0} max={100} className='outline-none md:py-2.5 w-28 py-2 px-3 rounded border border-gray-500' required />
        </div>
                ...
                <div className="p-4">
                  {chapter.chapterContent.map((lecture, lectureIndex) => (
                    <div key={lectureIndex} className="flex justify-between items-center mb-2">
                      ...
                      <img  src={assets.cross_icon}  onClick={()=> handleLecture('remove', chapter.chapterId, lectureIndex)} className='cursor-pointer' alt="cross-icon" />
                    </div>
                  ))}
...

          {
            ...
            <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
              <div className="bg-white text-gray-700 p-4 rounded relative w-full max-w-80">
                ...
                <img src={assets.cross_icon} alt="close"  onClick={()=> setShowPopup(false)} className='absolute top-4 right-4 w-4 h-4 cursor-pointer'/>
              </div>
            </div>
            )}
        </div>
